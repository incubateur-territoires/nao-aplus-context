-- Optimisation de la page /statistiques (lecture de analytics.v_report_fact).
--
-- 1) Pré-calcul du délai de résolution (création → COMPLETED/CLOSED) directement
--    dans la vue matérialisée. Avant, le graphique « Délai de traitement »
--    agrégeait TOUT public."ReportStatusHistory" en live à chaque chargement de
--    page. Désormais c'est calculé au refresh (toutes les 30 min) comme les
--    autres délais → le graphique devient un simple GROUP BY sur la vue.
--
-- 2) Index sur les colonnes d'IDENTIFIANT effectivement filtrées par la page
--    (areaId, authorOrganizationId, authorTeamId + GIN sur les tableaux d'ids
--    sollicités). Les index existants ne couvraient que les variantes par NOM,
--    inutilisables pour des filtres par id.
--
