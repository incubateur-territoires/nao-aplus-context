-- Ajoute la « prise en charge » à analytics.v_report_fact : premier geste de
-- l'opérateur, soit min(ReportStatusHistory.createdAt) où status IN
-- (IN_TREATMENT, COMPLETED), en trois colonnes calquées sur le trio inTreatment*.
-- Le seul passage en IN_TREATMENT ne suffit pas : en 2025, 72 % des signalements
-- passent directement en COMPLETED, 13 % par IN_TREATMENT, 15 % sont fermés
-- (CLOSED) par l'aidant sans réponse, donc jamais pris en charge.
