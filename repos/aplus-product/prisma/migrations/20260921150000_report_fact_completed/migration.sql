-- Remplace la « résolution » de analytics.v_report_fact par le « traitement » :
-- premier passage au statut COMPLETED (Traité) seul, en trois colonnes calquées
-- sur le trio takenInCharge*. L'ancienne CTE prenait aussi CLOSED, si bien que
-- les signalements fermés par l'aidant sans avoir été traités (15 % en 2025)
-- portaient un faux délai de traitement. Le traitement (impact) reste distinct
-- de la prise en charge (premier geste de l'opérateur, IN_TREATMENT ou COMPLETED).
-- L'objet prod est une MATERIALIZED VIEW rafraîchie par le cron
-- src/app/api/cron/analytics/refresh/route.ts et gérée à la main en prod :
-- cette migration s'y rejoue telle quelle, suivie d'un REFRESH.
...
completed AS (
  SELECT
    h."reportId",
    min(h."createdAt") AS completed_at
  FROM public."ReportStatusHistory" h
  WHERE h.status = 'COMPLETED'::public."ReportStatus"
  GROUP BY h."reportId"
),
in_treatment AS (
  SELECT
...
taken_in_charge AS (
  SELECT
    h."reportId",
    min(h."createdAt") AS taken_in_charge_at
  FROM public."ReportStatusHistory" h
  WHERE h.status = ANY (ARRAY[
    'IN_TREATMENT'::public."ReportStatus",
...
SELECT
  ...
  (cp.completed_at IS NOT NULL) AS "hasCompleted",
  (
    SELECT count(*)::bigint
    FROM generate_series(
      r."createdAt"::date::timestamptz,
      (cp.completed_at::date - 1)::timestamptz,
      INTERVAL '1 day'
    ) AS d(d)
    WHERE EXTRACT(dow FROM d.d) BETWEEN 1 AND 5
      AND d.d NOT IN (SELECT holiday_date FROM holidays)
  ) AS "completedDelayBusinessDays",
  ...
  tic.taken_in_charge_at AS "takenInChargeAt",
  (tic.taken_in_charge_at IS NOT NULL) AS "hasTakenInCharge",
  (
    SELECT count(*)::bigint
    FROM generate_series(
      r."createdAt"::date::timestamptz,
      (tic.taken_in_charge_at::date - 1)::timestamptz,
      INTERVAL '1 day'
    ) AS d(d)
    WHERE EXTRACT(dow FROM d.d) BETWEEN 1 AND 5
      AND d.d NOT IN (SELECT holiday_date FROM holidays)
  ) AS "takenInChargeDelayBusinessDays",
...
LEFT JOIN answer_flags af
  ON af."reportId" = r.id
LEFT JOIN completed cp
  ON cp."reportId" = r.id
LEFT JOIN in_treatment it
  ON it."reportId" = r.id
LEFT JOIN taken_in_charge tic
  ON tic."reportId" = r.id;
